...
func (f *flare) Create(pdata ProfileData, ipcError error) (string, error) {
	...
	// We don't try to fetch status and similar information when the flare is local (ie: we could not contact a
	// running agent and the flare is being generated by the CLI).
	if fb.IsLocal() {
		var msg []byte
		if ipcError != nil {
			msg = []byte(fmt.Sprintf("unable to contact the agent to retrieve flare: %s", ipcError))
		}
		f.log.Errorf("ADDING LOCAL FILE")
